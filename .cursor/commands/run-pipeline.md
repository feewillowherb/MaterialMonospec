...
id: run-pipeline
category: Workflow
description: Execute an existing pipelines/<slug> Graph; collect evidence; stop at human Gate
---

Run an **existing** Graph at `pipelines/<slug>/`. **Do not invent config.** Same split as OpenSpec apply vs propose.

**Input**: slug (directory name). Example: `/run-pipeline urban-weighing-list-observe`  
If omitted: infer from conversation, or list `pipelines/` (skip `_template` / `_templates`) and ask.

---

## Must read

- This slug’s `pipeline.md` + `config.yaml` (source of truth)
...
On user **pass** / **fail**: write only this run’s `acceptance.md`. Do not rewrite `pipelines/<slug>/acceptance.md` as the canonical pass.
...
```text
pipelines/<slug>/runs/<yyyy-MM-ddTHHmmss>/
  report.md
  acceptance.md
  <collector sinks…>
```
