/**
 * experimental — sand-addbatch-2026-02 (XNYH20251113001)
 * Expand Feb 2026 monthly tonnage → §2.2 addBatch **pending-POST JSON** (NO HTTP).
 *
 * pnpm exec tsx expand-february.ts --outDir <runDir> --seeds <yaml> --csvDir <pool>
...
function main() {
  ...
  if (!args.outDir || !args.seeds || !args.csvDir) {
    console.error(
      "Usage: pnpm exec tsx expand-february.ts --outDir <dir> --seeds <yaml> --csvDir <dir> [--addresses <yaml>] [--pointNumber <id>] [--preserveState <jsonl> --existingJsonDir <jsonDir>]",
    );
    process.exit(2);
  }
  ...
  const cfg = {
    ...
    seed: Number(args.seed) || 20260910,
  ...
  const summary = {
    graph: "sand-addbatch-2026-02",
    goal: "gov-sand-product-addbatch-2026-02",
