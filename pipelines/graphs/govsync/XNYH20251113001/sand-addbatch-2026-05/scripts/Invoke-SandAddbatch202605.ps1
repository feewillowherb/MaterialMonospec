...
<#
.SYNOPSIS
  Experimental cook: expand 2026-05 sand totals to pending-POST JSON (NO HTTP).
  Graph: pipelines/graphs/govsync/XNYH20251113001/sand-addbatch-2026-05/

  Default: full-month regenerate under Q5 (net~50 / tare~20 / gross~70).
  Optional -Preserve when 2026-05 durable state already has posted trips.
#>
[CmdletBinding()]
param(
    [string] $RunDir = "",
...
$DefaultPreserveState = Join-Path $SiteRoot "sand-addbatch-submit\state\2026-05\submit-state.jsonl"
$SubmitSeedsMonthDir = Join-Path $SiteRoot "sand-addbatch-submit\seeds\2026-05"
...
Write-Host "[sand-addbatch-2026-05] starting (submitEnabled=false, pending-POST JSON only)..."
...
Write-Host "[bind] pointNumber=$pointNumber month=5"
...
if ($usePreserve) {
    ...
    Write-Host "[bind] preserve=OFF (full May regenerate)"
}
...
try {
    ...
    if ($LASTEXITCODE -ne 0) {
        throw "expand-may.ts failed with exit $LASTEXITCODE"
    }
}
finally { Pop-Location }
...
if ($SyncSubmitSeeds.IsPresent) {
    ...
    $sourceMd = @"
# Frozen pointer — 2026-05
month: "2026-05"
sourceRunId: "$runId"
copiedFrom: graphs/govsync/XNYH20251113001/sand-addbatch-2026-05/runs/$runId
copiedAt: "$(Get-Date -Format 'yyyy-MM-ddTHH:mm:ssK')"
note: "May Q5 net~50/tare~20/gross~70. Do not mix with other months."
"@
    ...
    if (Test-Path -LiteralPath $indexMd) {
        ...
        $indexText = [regex]::Replace(
            $indexText,
            '(?m)(\|\s*2026-05\s*\|\s*`seeds/2026-05/`\s*\|\s*).*$',
            $indexReplacement
        )
