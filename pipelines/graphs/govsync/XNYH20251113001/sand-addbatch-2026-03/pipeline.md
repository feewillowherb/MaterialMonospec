# sand-addbatch-2026-03（govsync / XNYH20251113001）

## 目的 / Goal

生成 **2026 年 3 月** §2.2 `addBatch` **待 POST JSON**（含 `consigneeAddress`、`receivingTime` Q10、持久化 `dataNo` 台账），供用户验收后再另图提交。
...
Goal 槽：`gov-sand-product-addbatch-2026-03`（与其它月图 **互异 goal**；每月独立 Graph）

Status: **active**（2026-09-10 自 `sand-addbatch-2026-02` 派生）

路径：`pipelines/graphs/govsync/XNYH20251113001/sand-addbatch-2026-03/`
...
- `./config.yaml`（`submitEnabled: false`，`month: 3`，`outputFormat: json`）
- `./seeds/monthly-totals.yaml`（展开 `months[3]`）
...
- Node/TS：`./scripts/expand-march.ts`（experimental；`pnpm exec tsx`）
- Invoke：`./scripts/Invoke-SandAddbatch202603.ps1`（experimental）
...
- 3 月目标合计约 **110805.06** t（五家 `months[3]`）
...
powershell -ExecutionPolicy Bypass -File `
  pipelines/graphs/govsync/XNYH20251113001/sand-addbatch-2026-03/scripts/Invoke-SandAddbatch202603.ps1
```

默认：**全量生成**（3 月尚无已提交账本；勿默认套其它月 preserve）。  
可选：`-Preserve` + 提供 `state/2026-03/submit-state.jsonl` 后可续跑重拆。

命令：`/run-pipeline govsync/XNYH20251113001/sand-addbatch-2026-03`

## 人闸 / Gate

请验收待 POST 参数：`pass` / `fail` + 对象与原因。  
通过后拷贝到 **`sand-addbatch-submit/seeds/2026-03/source-run/<runId>/`**，再 HMAC POST（submit 图按月分仓；**不**与 1 月混跑）。
...
Output：`submit-params-json-ready`。下游 **`sand-addbatch-submit`** 切换 `source.month=2026-03` 后读取冻结源。
