...
<#
  Standalone GDXS20260520002 weighbridge probe for internal network.
  Params match pipelines/graphs/govsync/gdxs20260520002-weighbridge/config.yaml.
  Place snap image (test_pic.png or 16cfa8c48c427f193da61f7a6903f9d6.png) next to this script.
#>
[CmdletBinding()]
param(
    [switch] $SkipConfirm
...
$FixtureCandidates = @(
    (Join-Path $ScriptDir "16cfa8c48c427f193da61f7a6903f9d6.png"),
...
Write-Host "=== GDXS20260520002 weighbridge internal probe ===" -ForegroundColor Cyan
Write-Host "URL: $Url"

if (-not $FixturePath) {
    Write-Host "ERROR: missing snap image in: $ScriptDir" -ForegroundColor Red
    Write-Host "Expected: 16cfa8c48c427f193da61f7a6903f9d6.png (or test_pic.png)" -ForegroundColor Yellow
    exit 1
}
...
if (-not $SkipConfirm) {
    Write-Host ""
    Write-Host "WARNING: POST will WRITE a weighbridge record (license GDXS20260520002)." -ForegroundColor Yellow
