...
<#
  Standalone Xiaoshan product (finished-goods) probe for internal network.
  Params match pipelines/graphs/govsync/xiaoshan-product/config.yaml (no goodsWeight).
  Same schema as gate; only buildLicenseNo = L + "-02".
...
$Channel = "product"
$Slug = "xiaoshan-product"
$ProductSuffix = "-02"

$licenseRaw = "XNXS20250819001"
$buildLicenseNo = if ($licenseRaw.EndsWith($ProductSuffix)) { $licenseRaw } else { $licenseRaw + $ProductSuffix }
...
Write-Host "=== Xiaoshan product (finished-goods) internal probe ===" -ForegroundColor Cyan
Write-Host "URL: $Url"
...
if (-not $SkipConfirm) {
    Write-Host ""
    Write-Host "WARNING: POST will WRITE a product record to the government platform." -ForegroundColor Yellow
