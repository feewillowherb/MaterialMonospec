...
<#
.SYNOPSIS
  Experimental cook: XNXS20240725003 Xiaoshan weighbridge POST probe.
.DESCRIPTION
  Reads graph config.yaml and POSTs weighbridge payload with assets fixture.
...
$DefaultFixtureRel = "fixtures/b94c5e7e31e77b9eb42ec009e8deeb49.png"
...
function Get-YamlNestedScalar {
    ...
    $pattern = '(?ms)^\s{2}' + [regex]::Escape($Parent) + ':\s*\r?\n(?:^\s{4}.+\r?\n)*?^\s{4}' + [regex]::Escape($Key) + ':\s*(.+)$'
    if ($Text -match $pattern) {
        $v = $Matches[1].Trim()
        if ($v -match '^(.*?)\s+#') { $v = $Matches[1].Trim() }
        if ($v.StartsWith('"') -and $v.EndsWith('"')) {
...
Write-Host "[xnxs20240725003-weighbridge] experimental Invoke starting..."
...
if ([string]::IsNullOrWhiteSpace($slug)) { $slug = "xnxs20240725003-weighbridge" }

$fixtureRel = Get-YamlNestedScalar -Text $configText -Parent "snapImages" -Key "source"
if ([string]::IsNullOrWhiteSpace($fixtureRel)) { $fixtureRel = $DefaultFixtureRel }
$FixturePath = Join-Path $GraphRoot ($fixtureRel -replace '/', '\')
if (-not (Test-Path -LiteralPath $FixturePath)) {
    throw ("Missing fixture image: {0}. Run Copy-XnxsWeighbridgePackage.ps1 first." -f $FixturePath)
}
...
if ($channel -ne "weighbridge") {
    throw ("This graph only supports weighbridge; got channel={0}" -f $channel)
}
...
if (-not $SkipConfirm) {
    ...
    $ans2 = Read-Host ("POST weighbridge XNXS20240725003 will WRITE to government platform. Type YES to continue")
