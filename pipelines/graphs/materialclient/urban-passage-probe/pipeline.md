# urban-passage-probe（materialclient）
...
路径：`pipelines/graphs/materialclient/urban-passage-probe/`（见 [`pipelines/AGENTS.md`](../../../AGENTS.md)）
...
- 共享 seed：`pipelines/_shared/materialclient/Invoke-UrbanLprSeedSettings.ps1`
- 夹具：`../../govsync/xiaoshan-gate/fixtures/test_pic.jpg`
- 调研：`docs/2026-08-31-forge-hikvision-lpr-without-device/`

## 前置

1. 启动 **MaterialClient.Urban**（非主程序 exe）
2. `appsettings.json` → `MinimalWebHost:EnableOnStartup=true`，默认 `http://localhost:9961`
...
powershell -ExecutionPolicy Bypass -File `
  pipelines/graphs/materialclient/urban-passage-probe/scripts/Invoke-SeedSettings.ps1
```
...
```powershell
powershell -ExecutionPolicy Bypass -File `
  pipelines/graphs/materialclient/urban-passage-probe/scripts/Invoke-UrbanPassageProbe.ps1
```

或：`/run-probe-pipeline materialclient/urban-passage-probe`
