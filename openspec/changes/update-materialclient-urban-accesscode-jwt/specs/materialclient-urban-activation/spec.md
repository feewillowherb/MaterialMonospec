...
- **THEN** `ActivateUrbanAsync` SHALL 使用 `jwtToken` 调用 `CheckLicenseFromTokenAsync`
...
`ILicenseService` SHALL 提供 `ActivateUrbanAsync` 方法：调用 Urban API → 本地 JWT 验签（含 `machineCode`）→ Insert/Update `LicenseInfo`。写操作 SHALL 使用 `[UnitOfWork]`。

#### Scenario: 成功激活持久化

- **WHEN** `ActivateUrbanAsync` 完成且 JWT 验签通过
