...
- `DingSongAddr4` + HEX（`TF0`）可稳定解析 H610/H1320 帧并输出 kg 重量（**610 / 1320**）。
- 顶松 `DingSong` 行为不变。
...
- H610：`000610……` → **610 kg**（文件名 / 现场 Hex 610 kg）
- H1320：`001320……` → **1320 kg**（文件名 / 现场 Hex 1320 kg）

随后走现有 `ConvertWeight`（`ScaleUnit` kg/t）。

**备选**：整段 12 位 /100 或 /1000 — 与 Hex 610 / 1320 kg 语义不符，否决。

**注**：磁盘上 `_temp/H1320.txt` 当前重复帧载荷为 `001330`（按前 6 位会得到 1330）。实现与单测的 **golden 帧** 以契约 **1320 kg**（`001320……`）为准；若需与抓包文件逐字节一致，应先修正该 txt 后再对齐测试。
...
- `_temp/H1320.txt` 字节为 `001330` 而契约为 **1320 kg**：apply 前是否改写抓包文件，或仅用 golden `001320` 帧做测试？
- 若后续确认重量含小数位，再修订 D2。
