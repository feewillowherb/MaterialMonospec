...
`RecycleWeightMapper` SHALL 将 `WeighingRecord` 实体映射为 `RecycleTransportRecord` DTO，遵循 §2.2 接口字段要求。
...
`RecycleDataSyncService` SHALL 在 §2.2 接口返回 `Code == 200` 时将 `WeighingRecord.SyncStatus` 更新为 `SyncStatus.Synced`。

#### Scenario: 上报成功
- **WHEN** §2.2 接口返回 `{ "code": 200, "msg": "操作成功" }`
- **THEN** SHALL 将该记录 `SyncStatus` 设置为 `SyncStatus.Synced`
- **AND** SHALL 持久化到 SQLite

### Requirement: 同步失败重试与放弃
`RecycleDataSyncService` SHALL 在 §2.2 接口返回 `Code != 200` 时递增 `FailCount` 并记录 `FailMsg`；当 `FailCount >= MaxFailCount` 时 SHALL 将 `SyncStatus` 设置为 `SyncStatus.Failed`（放弃重试）。

#### Scenario: 可重试的失败
- **WHEN** §2.2 接口返回 `{ "code": 400, "msg": "参数错误" }`
