...
系统 SHALL 定义 `RecycleTransportRecord` 类，包含 §2.2 接口要求的全部字段。
...
系统 SHALL 定义 `RecycleApiResponse` 类，用于解析 §2.2 接口的响应。
