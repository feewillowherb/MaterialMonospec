...
- **新增"拉取到服务器"操作入口**：在查询条件面板中提供一键拉取按钮，调用 `PullLogsByDateAsync` 服务端 API 直接将客户端日志拉取到服务器磁盘（数据路径：Client → Hub → AppService → 磁盘，无浏览器中转）
- **重构拉取逻辑为服务端主动拉取**：将原浏览器 SignalR 中转拉取（Client → Hub → 浏览器 → Base64 → API → 磁盘）替换为服务端 `PullAndCacheAsync` API 调用，消除 Base64 中转开销
...
- `server-log-pull-api`: 完善 `DownloadBatchCachedAsync`、`DeleteCachedAsync`、`DeleteBatchCachedAsync` 三个未实现方法的业务逻辑；补全 `PullAndCacheAsync` 的 SignalR 文件传输；接入 `GetOnlineClientsAsync` 实际数据源；前端现已通过服务端 API 接入 `PullAndCacheAsync` 和 `PullLogsByDateAsync`
...
sequenceDiagram
    ...
    Note over Admin,Client: 方式A: 一键拉取到服务器（推荐）

    Admin->>UI: 选择客户端 + 日期, 点击"拉取到服务器"
    UI->>API: PullLogsByDateAsync(clientId, dateFolder)
    ...
    loop 每个文件（服务端直接拉取）
        ...
        Hub-->>API: OnServerFileChunk 回调
        API->>FS: 直接写入 ClientLogs/{ClientId}/{Date}/
    end
    API-->>UI: PullLogsByDateResult
    UI-->>Admin: 显示拉取结果 + 刷新已缓存列表
    ...
    Admin->>UI: 点击"下载"
    UI->>FS: 浏览器请求 GET /api/client-log/download/{id}
    FS-->>UI: FileStreamResult (文件流)
    UI-->>Admin: 浏览器下载文件
```
