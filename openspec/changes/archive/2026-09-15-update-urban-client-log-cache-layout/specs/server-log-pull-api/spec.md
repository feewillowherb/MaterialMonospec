...
拉取 API MUST 返回结构化成功与失败信息，供页面展示错误原因；同一批失败 MUST 同步写入服务端应用日志。
...
#### Scenario: 失败明细写入服务端日志

- **假设** `PullAndCacheAsync` 产生一条或多条 `Failures`
- **WHEN** 单文件失败被记录，以及整批拉取结束
- **THEN** 系统 SHALL 对每个失败项写入服务端 Warning 或 Error 日志，内容至少包含 `FileName`、`ClientId` 与 `Reason`
- **AND** 批次结束时 SHALL 再写入一条汇总日志，包含失败条数及逐条「文件名 — 原因」
- **AND** 日志中的 `Reason` SHALL 与返回给页面的 `Failures.Reason` 一致（可检索对照）
...
- **AND** 失败明细 SHALL 已由底层 `PullAndCacheAsync` 写入服务端应用日志
