...
- 拉取 API 返回结构化失败明细（文件名 + 原因）；`ClientLogs.razor` 在页面中展示汇总与逐条错误，不再只显示成功计数；同一批失败原因 MUST 同时写入服务端应用日志（可检索），不得只在页面可见。
...
- `server-log-pull-api`: 缓存目录布局改为 ProName/ClientId；拉取结果携带失败明细并写入服务端应用日志；缓存扫描与下载路径适配新布局并兼容旧目录。
