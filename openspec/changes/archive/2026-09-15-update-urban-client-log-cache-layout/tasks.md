...
- [x] 1.5 `PullAndCacheAsync` 改为返回 `PullAndCacheResult`；超时与客户端错误写入可区分的 `Failures.Reason`；每条失败打 Warning/Error，批次结束汇总逐条「文件名 — 原因」
...
- [x] 3.1 一键拉取 / 选择性拉取改用新结果类型；页面 alert 展示汇总与逐条失败原因（与服务端日志失败明细对照）
- [x] 3.2 已缓存区改为目录浏览：面包屑、单击进入、返回上级；调用 `BrowseCachedDirectoryAsync`
- [x] 3.3 文件层保留下载/删除（及既有批量能力若适用）；拉取成功后刷新当前目录视图

## 4. 验证

- [x] 4.1 手动验证：新拉取落在 `ProName/ClientId`；部分失败时页面与服务端日志均可见错误原因；目录可单击下钻；遗留缓存仍可浏览下载
