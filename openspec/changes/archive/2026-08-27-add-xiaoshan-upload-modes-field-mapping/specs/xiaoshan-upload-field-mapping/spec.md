...
#### Scenario: Config UI labels no-source skip

- **WHEN** the configuration UI displays a static field that is optional and has no configured value and no automatic record source
- **THEN** the UI SHALL indicate that the field will be skipped at upload time due to missing data source

### Requirement: Upload path logs skipped fields

When the upload or field-mapping path processes a weighing event against Xiaoshan upload configuration, it SHALL emit a structured log entry for each skipped optional field including mode, field name, and skip reason.

#### Scenario: Skip logged on upload path

- **WHEN** upload assembly skips an optional field because no source is available
- **THEN** the system SHALL write a log entry containing the mode, field name, and skip reason
