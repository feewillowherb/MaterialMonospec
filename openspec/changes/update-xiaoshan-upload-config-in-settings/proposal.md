...
- 在共享 `SettingsWindow` 左侧导航**新增「城管配置」**，**默认第一位**；**仅 MaterialClient.Urban** 可见。
- 将既有萧山上报编辑面（三模式、静态字段等）迁入「城管配置」；**移除**独立菜单「上报配置」与独立配置窗主路径。
- 系统设置「保存」时：经 **`ILocalEventBus`** 将城管配置**推送到服务端**；ViewModel 不直调 Refit/API。推送结果写入应用日志。
- **一致性**：服务端为准。推送成功 → UI 以服务端返回（或再 Get）为准；**推送失败或未推送成功 → 舍弃本地编辑**，拉取并展示服务端配置。
- **移除客户端 `configVersion` 相关行为**（UI 展示、expectedVersion 冲突流、本地 version 镜像依赖）。
- **移除客户端对 `XiaoshanUploadConfigCaches` 的使用**；因未发布上线，**直接删除**相关 EF Migration、实体与 DbSet（不另开 Drop 迁移）。本地编辑态仅用内存或既有 `SystemSettings` JSON 节承载至保存推送为止。
- **不做**：UrbanManagement 管理端改版；GovSync 实际上报改造；主程序/Recycle 增加城管配置。服务端若仍存 ChangeLog/`ConfigVersion` 列，本 change **不要求客户端消费**。
