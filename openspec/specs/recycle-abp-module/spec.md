...
`MaterialClientRecycleModule` SHALL 在 `ConfigureServices` 中注册 `RecyclePollingBackgroundService` 作为 ABP 后台 Worker。

#### Scenario: 轮询服务注册
- **WHEN** `RecycleSync:Enabled` 为 `true`
...
- **WHEN** Recycle 应用启动完成 ABP 初始化且授权检查通过
- **THEN** SHALL 显示称重主界面
- **AND** SHALL 通过 ABP 容器初始化称重管线服务

#### Scenario: 授权失败退出
- **WHEN** Recycle 应用启动且授权检查失败
- **THEN** SHALL 显示「软件未授权」提示对话框
