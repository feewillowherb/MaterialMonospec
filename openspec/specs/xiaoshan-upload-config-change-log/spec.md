# Xiaoshan Upload Config Change Log Specification

## Purpose

记录每次被接受的萧山上报配置写入，供按项目查询与管理端展示。变更日志不替代 `configVersion` 作为运行时一致性键。

## Requirements

### Requirement: Server records a change log entry on each accepted write

When an authoritative Xiaoshan upload configuration write is accepted, the system SHALL append a change log entry in the same unit of work. Each entry SHALL record at least: project id, resulting `configVersion`, source (Client, Server, or Unknown), actor identifier, a short summary of the change, and creation time. Change logs MUST NOT replace `configVersion` as the runtime consistency key. Legacy merge writes SHALL indicate a legacy merge in the summary.
