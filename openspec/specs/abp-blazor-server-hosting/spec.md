...
`UrbanManagement.Core.csproj` MUST 引用 `Volo.Abp.AspNetCore.Components.Server` NuGet 包以启用 ABP Blazor Server 集成。

#### Scenario: Core 项目包含 Blazor 包
...
`UrbanManagementCoreModule` MUST 在 `ConfigureServices` 中注册 Blazor Server 服务。

#### Scenario: AddServerSideBlazor 注册
