...
Each consuming app MUST be able to configure which devices appear in the status bar.

#### Scenario: Urban device set
- **WHEN** MaterialClient.Urban initializes DeviceStatusBarViewModel
- **THEN** it SHALL show indicators for: scale, camera, LPR device
- **AND** SHALL NOT show printer or sound device indicators (Urban does not use them)
