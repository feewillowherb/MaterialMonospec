...
Defines server-to-client sync of Web weighing-record approvals: nullable sync timestamps on `UrbanWeighingRecord`, SignalR push after `ApproveAsync`, client pull/ACK APIs, and a relaxed conflict policy when client and server approval overlap.
## Requirements
