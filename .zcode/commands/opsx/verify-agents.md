...
4. **Build review file scope**
   ...
   Write the union of paths to a temp file, e.g. `.zcode/.opsx-verify-<name>-files.txt` (create `.zcode` if needed). Exclude paths that do not exist on disk.
...
6. **Mechanical validation**
   ...
   powershell -ExecutionPolicy Bypass -File scripts/validate-agents-implementation.ps1 `
     -ChangeName "<name>" `
     -FileListPath ".zcode/.opsx-verify-<name>-files.txt" `
