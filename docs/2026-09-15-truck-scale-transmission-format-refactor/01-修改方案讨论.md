...
| ScaleType     | tf0           | tf1                                        |
| ------------- | ------------- | ------------------------------------------ |
| Yaohua        | 现有连续帧解析（迁出）   | **新建**指令应答（参考 Demo）                        |
...
- **tf0**：各设备真实解析（从巨石迁出）。
- **tf1**：仅耀华真实实现；**其余全部** `ScaleType`**（含 TestMode）** 走 `NotSupportedTransmissionHandler`（调用抛不支持异常，禁止返回 0 重量假装在线）。
...
│ 各 ScaleType  │  │ 仅耀华指令应答   │  │ 非耀华含 TestMode 共享    │
│ 真实帧解析    │  │                 │  │ 调用即抛「不支持」        │
└───────────────┘  └─────────────────┘  └──────────────────────────┘
```
...
sequenceDiagram
  ...
  alt 非耀华且 Format1
    ...
    F->>S: Open + 绑定 DataReceived 或启动 Poll
    F->>P: Bind / OnStart
    F-->>UI: true
    loop 运行中
      S-->>F: bytes / poll tick
      F->>P: OnDataReceived 或 PollAsync
      P-->>F: weight?
...
| `IScaleTransmissionProtocol` | 帧解析 / 轮询指令 / Supports / 不支持则抛                       | 持有长期 SerialPort 生命周期（建议由门面注入上下文） |
...
| 资产                | 角色                                   |
| ----------------- | ------------------------------------ |
| Demo tf0 / tf1 窗口 | 协议对照与肉眼验收；**不要**把 Demo 代码直接拷进生产门面    |
| `docs/Device/耀华…` | tF 语义权威；生产 tf1 指令集以实现前再从 Demo + 手册核对 |
...
| Δ2    | 耀华 **tf1** 生产实现；设置页可选 tf1                | 仅耀华       |
| Δ3    | 删除开关与旧巨石路径；测试补齐                          | Promote 前 |


若 Δ2 指令集 / 地址位事实未齐：按 strategic-fallback **显式挂起** Yaohua tf1，不得假实现。
