...
| 毛皮净 | 二次过磅 / 状态位 / 拓展帧 + 软件记账 | **优先**连续查询帧内皮/毛/净；任一无效即降级旧稳定模式 |
...
| 重量取值 | **优先**连续查询皮/毛/净；任一无效 → **降级旧稳定模式**（实时流仍在） |
...
2. **过磅记账优先采用连续查询解析出的皮重 / 毛重 / 净重**（仪表侧分量）——**三者必须全部有效**才走此路径。
3. 皮重、毛重、净重中 **只要有一个无效**（未给出 / 空 / 解析失败 / 协议约定的无效哨兵），即 **降级为现网旧稳定模式**：
...
flowchart TD
  ...
  Frame --> Comp{皮/毛/净三者是否全部有效?}
  Comp -->|全部有效| Prefer[优先采用仪表皮毛净]
  Comp -->|任一无效| Stable[降级旧稳定模式]
  Live --> Stable
  Prefer --> Biz[过磅 / UI]
  Stable --> Biz
```
...
| 层级 | 何时用 | 说明 |
|------|--------|------|
| L1 连续查询皮毛净 | 皮、毛、净 **三者全部有效** | 产品优先路径 |
| L2 旧稳定模式 | 皮/毛/净 **任一无效** | 现网稳定窗口；依赖同一实时重量流 |
| 禁止 | — | 为取重向设备写查询命令 |
...
**一句话**：生产耀华 Type1 = **连续查询、不发查询命令**；重量 **优先皮/毛/净，任一无效即降级旧稳定模式**（实时流仍可用）；`CommunicationParameter` 切 TF 重置，耀华默认 **A**。
