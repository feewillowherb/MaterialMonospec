...
1. ViewModel 未使用 `[Reactive]` Source Generator（已引入包但未使用）
2. ViewModel/Window 未标记 `ITransientDependency`（ABP 自动扫描 vs 手动注册）
3. Window 注册生命周期不一致（`Singleton` vs MaterialClient 的 `Transient` + `ITransientDependency`）
4. 命名空间风格不一致（block scoped vs file scoped）
5. Program 类缺少 `internal sealed` 修饰符
6. 事件处理方法命名约定不一致（`OnXxxClick` vs `OnXxxButtonClick`）

### P2 — UI 样式缺失

1. App.axaml 缺少命名颜色资源（全部硬编码色值）
2. 缺少 SemiTheme / Ursa 主题
3. 缺少 DataGrid 选中行样式
4. 窗口 `SystemDecorations` 不一致（`None` vs `BorderOnly`）
5. 缺少 ComboBox / CalendarDatePicker focus 样式
6. 缺少窗口 Icon
7. PreConfigureServices 缺少 `#if DEBUG` UserSecrets 支持

### P3 — 改进建议

1. 内联 DataTemplate 应提取为独立控件（减少主窗口 XAML 复杂度）
2. FontFamily 字符串应提取为共享资源
3. 手写表头应改用 DataGrid（已有全局 DataGrid 样式定义）
