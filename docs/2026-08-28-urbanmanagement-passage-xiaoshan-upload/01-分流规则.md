...
优先 **两条进出上云入口**：**来自 LPR 卡口**、**来自 LPR 成品**（各走 ApplicationService；地磅保持现网 `UrbanWeighingRecordAppService.ReceiveAsync`）。服务端用来源本身分流，避免单一 DTO 里靠可选重量猜测。

若必须共用一个 HTTP 入口：请求体 MUST 带显式判别（命名 `record`，例如来源 = 地磅 LPR | LPR 卡口 | LPR 成品），**禁止** tuple、禁止用 `TotalWeight == 0` 当卡口。
